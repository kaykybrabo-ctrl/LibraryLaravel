<template>
  <div class="container">
    <div style="display:flex; justify-content: space-between; align-items:center; margin-bottom: 12px;">
      <h2>{{ $t('users.profileTitle') }}</h2>
      <button class="btn btn-small btn-secondary" @click="$emit('goBack')" style="width:auto;">{{ $t('users.backToUsers') }}</button>
    </div>
      ...
      <div style="display: flex; align-items: center; gap: 20px; margin-bottom: 20px;">
        ...
        <div>
          ...
          <span style="display: inline-block; margin-top: 5px; padding: 3px 8px; background: #e9ecef; border-radius: 12px; font-size: 0.8rem; color: #495057;">
            {{ selectedUser.is_admin ? $t('users.roleAdmin') : $t('users.roleUser') }}
          </span>
          <p style="margin: 8px 0 0 0; color:#777; font-size:0.85rem;">
            {{ selectedUser.is_admin ? $t('users.adminAccountDescription') : $t('users.readerAccountDescription') }}
          </p>
        </div>
      </div>

      <div v-if="selectedUserActiveLoans.length > 0">
        <h3 style="margin: 20px 0 10px 0; color: #162c74;">{{ $t('users.userLoansTitle') }}</h3>
        <div v-for="loan in selectedUserActiveLoans" :key="loan.id" style="padding: 15px; border-bottom: 1px solid #eee;">
          ...
          <p style="margin: 0; color: #666; font-size: 0.9rem;">{{ $t('loans.borrowedAt') }} {{ formatDate(loan.loan_date) }}</p>
          <p style="margin: 5px 0 0 0; color: #666; font-size: 0.9rem;">{{ $t('loans.dueAt') }} {{ formatDate(loan.return_date) }}</p>
        </div>
      </div>

      <div style="margin-top: 30px;">
        <h3 style="margin: 0 0 10px 0; color: #162c74;">{{ $t('users.userFavoriteBookTitle') }}</h3>
        <div v-if="selectedUserFavoriteBook" style="background: #f8f9fa; padding: 15px; border-radius: 8px; display:flex; gap:16px; align-items:center;">
          ...
          <div style="flex:1;">
            ...
            <p style="margin: 0; color: #666; font-size: 0.9rem;">{{ $t('common.by') }}
        ...
        <p v-else style="color: #666;">{{ $t('users.noFavoriteBook') }}</p>
      </div>
    </div>
  </div>
</template>
